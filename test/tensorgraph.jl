...
@testset verbose = true "Tensorgraph" begin
    using FeynmanDiagram.ComputationalGraphs:
        TensorGraph, einsum
    g1 = TensorGraph([], dim=[5, 10, 15])
    g2 = TensorGraph([], dim=[15, 5, 20, 25])
    g3 = TensorGraph([], dim=[5, 10])

    g4 = einsum(g1, [1, 2, 3], g2, [3, 1, 4, 5], [2, 4, 5])
    @test g4.dim == [10, 20, 25]
    ...
    @test g7.dim == [10, 5, 20, 25]

    #test from numpy.einsum
    a = collect(reshape(0:24, 5, 5))
    b = collect(0:4)
    ga = TensorGraph([], dim=collect(size(a)))
    gb = TensorGraph([], dim=collect(size(b)))

    @tensor result[i] := a[i, j] * b[j]
    @test collect(size(result)) == einsum(ga, [0, 1], gb, [1]).dim
    ...
    @test collect(size(result)) == einsum(ga, [0, 1, 2], gb, [1, 0, 3], [2, 3]).dim
end