...
abstract type VacuumDiag <: DiagramType end
abstract type TadpoleDiag <: DiagramType end
abstract type FermiPropagator <: DiagramType end
abstract type BosePropagator <: DiagramType end
abstract type FermiSelfEnergy <: DiagramType end
abstract type BoseSelfEnergy <: DiagramType end
abstract type VertexDiag <: DiagramType end
abstract type GncDiag <: DiagramType end
abstract type GnDiag <: DiagramType end
...
export VacuumDiag, TadpoleDiag, GncDiag, GnDiag
# export Wirreducible, Girreducible, NoBubble, NoHartree, NoFock, Proper
export NoBubble, NoHartree, NoFock, Proper
...
export GreenDiagram, ExternalVertice, addSubDiagram!, toDataFrame
...
#     let
#         para = DiagParaF64(type=VertexDiag, extNum=4, innerLoopNum=2, hasTau=true)
...
#         para = DiagParaF64(type=FermiSelfEnergy, innerLoopNum=2, hasTau=true)
#         Parquet.build(para)  # this will force precompilation
#         para = DiagParaF64(type=FermiPropagator, innerLoopNum=2, hasTau=true)
#         Parquet.green(para)  # this will force precompilation
#         para = DiagParaF64(type=BoseSelfEnergy, innerLoopNum=2, hasTau=true)
#         # Parquet.polarization(para)  # this will force precompilation
#         Parquet.build(para)  # this will force precompilation
#         para = DiagParaF64(type=VertexDiag, extNum=3, innerLoopNum=2, hasTau=true)
