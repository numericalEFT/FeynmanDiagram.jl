...
"""
function vertex3(para::DiagPara,
    ...
    t0 = para.firstTauIdx
    vertex3 = DataFrame(response=Response[], extT=Tuple{Int,Int,Int}[], diagram=Graph{Ftype,Wtype}[])
    ...
    if isempty(vertex3) == false
        vertex3 = mergeby(vertex3, [:response, :extT]; name=name,
...
function _properVer3Para(p::DiagPara, q)
