...
function polarization(para::DiagPara, extK=getK(para.totalLoopNum, 1), subdiagram=false; name=:Π, resetuid=false,
    ...
    polar = DataFrame(response=Response[], extT=Tuple{Int,Int}[], diagram=Graph{Ftype,Wtype}[])
    ...
    if isempty(polar) == false
        polar = mergeby(polar, [:response, :extT]; name=name,
...
function _properPolarPara(p::DiagPara, q)
