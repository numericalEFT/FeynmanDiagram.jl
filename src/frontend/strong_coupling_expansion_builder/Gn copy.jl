...
# 	equal_times_site = Dict{Int, Int}()
# 	equal_times_diffsite = Dict{Int, Int}()
...
# 			if i == j
# 				if !haskey(equal_times_site, ti)
# 					equal_times_site[ti] = ti
# 				end
# 				if !haskey(equal_times_site, tj)
# 					equal_times_site[tj] = equal_times_site[ti]
# 				end
# 			else
# 				if !haskey(equal_times_diffsite, ti)
# 					equal_times_diffsite[ti] = ti
# 				end
# 				if !haskey(equal_times_diffsite, tj)
# 					equal_times_diffsite[tj] = equal_times_diffsite[ti]
# 				end
# 			end
# 		end
# 	end
...
# 			t = [equal_times_site[ti] for ti in t]
# 			bareGId = BareGreenNId(para, orbital = o, t = t, r = r, creation = c)
# 			if isnothing(bareGreenN)
# 				bareGN = Graph([], properties = bareGId, name = Symbol("gn$(length(t))"), factor = prefactor(o[[2m - 1 for m in 1:length(ind)]], num_orbital))
# 			else
...
# 		end
# 		return Graph(gn, properties = property, operator = Prod(), name = name)
# 	elseif any(length.(Gn) .== 4)
# 		# println("len of Gn: ", length.(Gn), "site:", Set(site))
# 		property = VacuumId(para)
# 		for i in 1:num_orbital
# 			isempty(Gn[i]) && continue
# 			uniqueR = Set(site)
# 			println("hopping site: ", [_g.properties.site for _g in Gn[i]], "t:", [_g.properties.extT for _g in Gn[i]])
# 			# push!(dets_g, Graph(Gn[i], operator = Det(), name = :det))
# 			gn = [Gn[i][1], Gn[i][4]]
# 			# println(equal_times_site)
# 			# println(equal_times_diffsite)
# 			for r in uniqueR
# 				ind = findall(x -> x == r, site)
# 				t = collect(Iterators.flatten(extT[ind]))
# 				o = collect(Iterators.flatten(orbital[ind]))
...
# 				_t = [equal_times_site[ti] for ti in t]
# 				bareGId = BareGreenNId(para, orbital = o, t = _t, r = r, creation = c)
# 				bareGN = bareGreenN(bareGId) * prefactor(o[[2m - 1 for m in 1:length(ind)]], num_orbital)
# 				push!(gn, bareGN)
# 			end
# 			final_g = [Graph(gn, properties = property, operator = Prod(), name = name)]
...
# 				_t = [equal_times_diffsite[ti] for ti in t]
# 				bareGId = BareGreenNId(para, orbital = o, t = _t, r = r, creation = c)
# 				bareGN = bareGreenN(bareGId) * prefactor(o[[2m - 1 for m in 1:length(ind)]], num_orbital)
# 				push!(gn, bareGN)
# 			end
# 			push!(final_g, Graph(gn, properties = property, operator = Prod(), name = name))
# 			return Graph(final_g, subgraph_factors = [1, -1], properties = property, operator = Sum(), name = name)
# 		end
# 	else
# 		println(Gn)
# 		error("incorrect")
