...
function _taylorexpansion!(graph::G, taylormap::Dict{Int,TaylorSeries{G}}, var_dependence::Dict{Int,Vector{Bool}}=Dict{Int,Vector{Bool}}()) where {G<:Graph}
    ...
    elseif isleaf(graph)
        ...
        taylormap[graph.id] = apply(graph.operator, [taylorexpansion!(sub, taylormap, var_dependence) for sub in graph.subgraphs], graph.subgraph_factors)
        return taylormap[graph.id]
    end
end
...
"""
function taylorexpansion!(graph::G, taylormap::Dict{Int,TaylorSeries{G}}=Dict{Int,TaylorSeries{G}}(), var_dependence::Dict{Int,Vector{Bool}}=Dict{Int,Vector{Bool}}()) where {G<:Graph}
    _taylorexpansion!(graph, taylormap, var_dependence)
end


function taylorexpansion!(graphs::Vector{G}, taylormap::Dict{Int,TaylorSeries{G}}=Dict{Int,TaylorSeries{G}}(), var_dependence::Dict{Int,Vector{Bool}}=Dict{Int,Vector{Bool}}()) where {G<:Graph}
    for graph in graphs
        _taylorexpansion!(graph, taylormap, var_dependence)
    end
end
