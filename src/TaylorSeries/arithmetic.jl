...
function Base.zero(g::TaylorSeries{T}) where {T}
	_zero = TaylorSeries{T}()
	_zero.coeffs[zeros(Int, get_numvars())] = zero(T)
	return _zero
end
...
function LinearAlgebra.det(M::AbstractMatrix{<:TaylorSeries})
	n = size(M, 1)
	...
	det_sum = zero(M[1, 1])
	for j in 1:n
		minor = M[2:end, vcat(1:j-1, j+1:n)]
		cofactor = (-1)^(1 + j) * det(minor)
		det_sum += M[1, j] * cofactor
	end
	return det_sum
end
