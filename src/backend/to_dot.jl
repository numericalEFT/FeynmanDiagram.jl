...
function to_dotstatic(::Type{ComputationalGraphs.Prod}, id::Int, factor::F, subgraphs::Vector{Graph{F,W}}, subgraph_factors::Vector{F}) where {F,W}
    ...
    if factor != 1
        ...
        # arrow_temp *= "factor$(id)->g$(id)[arrowhead=vee,]\ng$(id)_t->g$(id)[arrowhead=vee,]\n"
        # node_temp *= opr_fac * node_str
        opr_node = "g$id[shape=box, label = <($factor)<b>&otimes;</b>>, style=filled, color=violet , fontsize=18, width = 0.8, height = 0.4]\n"
    else
        opr_node = "g$id[shape=box, label = <<b>&otimes;</b>>, style=filled, color=violet, fontsize=18, width = 0.5, height = 0.4]\n"
    end
