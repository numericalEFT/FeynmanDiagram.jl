...
"""
function to_python_str(graphs::AbstractVector{<:AbstractGraph};
	root::AbstractVector{Int} = [id(g) for g in graphs], name::String = "eval_graph", in_place::Bool = true)
	head = ""
	body = ""
	leafidx = 0
...
function compile_Python(graphs::AbstractVector{<:AbstractGraph}, filename::String;
	root::AbstractVector{Int} = [id(g) for g in graphs], func_name = "eval_graph", in_place::Bool = true)
	py_string, leafmap = to_python_str(graphs, root = root, name = func_name, in_place = in_place)
