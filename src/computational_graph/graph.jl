...
mutable struct Graph{F,W} # Graph
    ...
    """
    function Graph(vertices::Vector{OperatorProduct}; external=[], topology=[], subgraph=[],
        name="", type=:generic, operator::Operator=Sum(), orders=zeros(Int, 16),
        ftype=_dtype.factor, wtype=_dtype.weight, factor=one(ftype), weight=zero(wtype)
    )
        return new{ftype,wtype}(uid(), name, type, orders, external, vertices, topology, subgraph, operator, factor, weight)
    end
    ...
    function Graph(extV::AbstractVector, intV::AbstractVector; topology=[], subgraph=[],
        ...
        return new{ftype,wtype}(uid(), name, type, orders, ext, vertices, topology, subgraph, operator, factor, weight)
    end
end
