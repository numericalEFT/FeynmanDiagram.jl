"""
    function sigmaGΓ3(para, extK = DiagTree.getK(para.totalLoopNum, 1), subdiagram = false; name = :Σ, resetuid = false, blocks::ParquetBlocks=ParquetBlocks())
...
"""
function sigmaGΓ3(para::DiagPara{W}; extK=DiagTree.getK(para.totalLoopNum, 1), subdiagram=false, name=:Σ, resetuid=false, blocks::ParquetBlocks=ParquetBlocks()) where {W}
    return sigmaGΓ3(para, extK, subdiagram; name=name, resetuid=resetuid, blocks=blocks)
end
function sigmaGΓ3(para::DiagPara{W}, extK=DiagTree.getK(para.totalLoopNum, 1), subdiagram=false; name=:Σ, resetuid=false, blocks::ParquetBlocks=ParquetBlocks()) where {W}
    resetuid && uidreset()
