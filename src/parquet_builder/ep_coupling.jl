...
function ep_bubble!(ver4df::DataFrame, para::DiagPara{W}, legK, chans::Vector{TwoBodyChannel}, partition::Vector{Int}, name::Symbol, blocks::ParquetBlocks,
    ...
    Rver = DataFrame(response=Response[], type=AnalyticProperty[], extT=Tuple{Int,Int,Int,Int}[], diagram=Diagram{W}[])
    bareVer4(Rver, rPara, RLegK, [Di,], false) # the external tau is right aligned
    Rver = merge_vertex4(rPara, Rver, :bare, RLegK, W)

    @assert isempty(Rver) == false

    for ldiag in Lver.diagram
        for rdiag in Rver.diagram
            LvT, RvT = ldiag.id.extT, rdiag.id.extT
            extT, G0T, GxT = tauBasis(PHr, ldiag.id.extT, rdiag.id.extT)
