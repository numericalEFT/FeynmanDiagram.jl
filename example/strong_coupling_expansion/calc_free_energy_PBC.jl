...
function integrand(idx, vars, config)
    ...
    for (i, lftype) in enumerate(leafType[idx])
        ...
        elseif lftype == 4  # BareHoppingId
            ...
            # leafval[idx][i] = green_counterterm_PBC(para, τ, r1, r2, orbital, order) * (-1)
        ...
        elseif lftype == 5  # BareGreenNId
            ...
            # idx == 2 && println(sites_i, sites_o, " r_dict: ", r_dict)
            if isnothing(r_dict)
                leafval[idx][i] = 0.0
                continue
            end
            ...
            for (loc_i, loc_o) in values(r_dict)
                # if idx == 2
                #     print("($(leafτ_i[idx][i][loc_i][1]) $(leafτ_o[idx][i][loc_o][1])) ")
                # end
                ...
                # leafval[idx][i] *= Green.Gn(model, _gn) * (-1)^(length(τ) ÷ 2)
                # leafval[idx][i] *= Green.Gn(model, _gn) / factorial(length(τ) ÷ 2)^2
