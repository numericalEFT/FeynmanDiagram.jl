...
function double_occupancy(model, para::ParaMC, diagram; neval=1e6, print=0, dtype=ComplexF64, kwargs...)
    ...
    root = zeros(dtype, 1)
    T_doublon = Continuous(0.0, para.β; adapt=true, alpha=2.0)
    T = Continuous(0.0, para.β; offset=1, adapt=true, alpha=2.0)
    T.data[1] = 0.0
    # R = Discrete(1, para.Lx, adapt=true, alpha=2.0)
    R = Discrete(1, para.Lx; offset=1, adapt=true, alpha=2.0)
    R.data[1] = 1
