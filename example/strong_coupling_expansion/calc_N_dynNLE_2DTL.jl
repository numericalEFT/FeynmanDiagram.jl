...
function integrand(idx, vars, config)
    ...
    for (i, lftype) in enumerate(leafType[idx])
        ...
        elseif lftype == 4            # BareHoppingId / counterterm-like line
            ...
            # if Rsample in abs.(r_o) || Rsample in abs.(r_i)
            #     println("bare_line_fast on Rsample box surface! ", leafval[idx][i])
            # end
        elseif lftype == 5      # BareGreenNId: product of local GreenN blocks
            ...
            deriv_order = leafOrders[idx][i][2]
            if deriv_order == 0
            ...
            elseif deriv_order == 1
                ...
                error("this order $deriv_order not implemented!")
            end
        else
...
function density(model, para::ParaMC, diagram;
    neval=1e6,
    print=0,
    dtype=ComplexF64,
    kwargs...)
    ...
    dof = [[1, p.totalTauNum - 1,   # number of non-root τ's
    ...
    config = Configuration(; var=(T_N, T, R), dof=dof, obs=obs, type=dtype,
...
function density_MC(model, para::ParaMC;
    neval=1e6,
    ...
    _density, result = density(model, para, diagram;
        neval=neval,
        neighbor=_neighbor,
