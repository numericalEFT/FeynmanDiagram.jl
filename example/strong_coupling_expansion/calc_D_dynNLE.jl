...
# function measure(idx, vars, obs, weights, config)
#     leafval, leafType, leafOrders, leafSites, leafτ_i, leafτ_o, leaforbitals_i, leaforbitals_o = config.userdata[4]
#     varT, varRx = vars

#     for (i, lftype) in enumerate(leafType[idx])
#         if lftype == 5  # BareGreenNId
#             Np = Int(length(leafSites[idx][i]) / 2)
#             sites_i = varRx[leafSites[idx][i][1:Np]]
#             sites_o = varRx[leafSites[idx][i][Np+1:end]]
#             idx_i = findfirst(item -> item == 1, sites_i)
#             idx_o = findfirst(item -> item == 1, sites_o)
#             if !isnothing(idx_i)
#                 τ = varT[leafτ_i[idx][i]][idx_i]
#                 orbital = leaforbitals_i[idx][i][idx_i]
#             elseif !isnothing(idx_o)
#                 τ = varT[leafτ_o[idx][i]][idx_o]
#                 orbital = leaforbitals_o[idx][i][idx_o]
#             else
#                 continue
#             end
#         end
...
function double_occupancy(model, para::ParaMC, diagram; neval=1e6, print=0, dtype=ComplexF64, kwargs...)
    ...
    R = Discrete(1, para.Lx; adapt=true, alpha=3.0)
