push!(LOAD_PATH, pwd())
using Atom, FFTW #QuantumStatistics,
using Lehmann
...
function green2(rep::Symbol, m, grid = nothing, rtol = 1e-10)
	...
	println(Λ)
	dlr = DLRGrid(Float64(Λ), Float64(m.β), Float64(rtol), true) # isFermi=true
	...
	coeff = tau2dlr(dlr, Float64.(g2t), axis = 3)

	if rep == :ωn
		g2w = dlr2matfreq(dlr, coeff, grid, axis = 3)
		...
		g2t = dlr2tau(dlr, coeff, grid, axis = 3)
...
function propagator(rep::Tuple{Symbol, Symbol}, m, L, disperion::Function, t, grid = nothing, rtol = 1e-10)
	...
	Λ = 2 * maximum(abs.(m.E)) * m.β # 2*(energy scale)
	println(Λ)
	dlr = DLRGrid(Float64(Λ), Float64(m.β), Float64(rtol), true)
	...
	coeff = matfreq2dlr(dlr, gwk, axis = 3)

	if rep[1] == :ωn
		return dlr2matfreq(dlr, coeff, grid, axis = 3)
	else
		return dlr2tau(dlr, coeff, grid, axis = 3)
	end
end
